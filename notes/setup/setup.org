...
# @see: https://docs.astral.sh/uv/guides/
...
[ -f ~/.python-version ]   && uv python install --verbose $(cat ~/.python-version) 2>&1 \
        | tee /tmp/uv-python-install-$(date -Isec).log; ls -l /tmp/pyenv-install-*
...
Y_PY_PYENV_CHECK=1 check_pyenv
...
grep 'NVM_DIR' ~/.bashrc ||  cat << 'EOF' >> ~/.bashrc
...
function install_node() {
    ...
    # fnm
    FNM_PATH="$FNM_ROOT"
    ln -sf "$FNM_ROOT/fnm" ~/.local/bin/fnm
...
Y_JS_NODE_INSTALL=1  install_node
...
# lab (public)
jupyter lab --notebook-dir=notebooks --no-browser --ip=0.0.0.0 --port=8888 --ServerApp.allow_remote_access=true
